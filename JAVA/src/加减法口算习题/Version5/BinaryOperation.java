...
abstract class BinaryOperation {
    ...
    protected void generateBinaryOperation(char anOperator) {
        ...
        } while (!(checkingCalculation(result)) || !(isInteger(left, right)));
        left_operand = left;
        right_operand = right;
        operator = anOperator;
    }

    abstract boolean isInteger(int left, int right);
...
class AdditionOperation extends BinaryOperation {
    ...
    @Override
    boolean isInteger(int left, int right) {
        return true;
    }
   ...
   /*   Entry将键值对的对应关系封装成了对象，即键值对对象，这样我们在遍历Map集合时，
        ...
        for (Map.Entry<Integer, Integer> entry1 : hashMap.entrySet()) {
            ...
            if (!m1value.equals(m2value)) {
                ...
                System.out.println("恭喜你，第" + entry1.getKey() + "题回答正确");
            }
        }
        return count;
    }
...
//        exercise.generateMultiplicationExercise(30);
        //生成operationCount个除法算式
//        exercise.generateDivisionExercise(200);
